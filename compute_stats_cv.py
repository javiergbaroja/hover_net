...
def run_nuclei_type_stat(pred_dir, true_info, root_dir, merge_epit, type_uid_list=None, exhaustive=True, only_epit=True,):
    ...
    file_list = natsorted(file_list)  # ensure same order [1]
    ...
    # file_list = [f for f in file_list if 'TMA' in f]
...
if __name__ == "__main__":
    ...
    if args.mode == "type":
        if args.cv is None:
            simple, full = run_nuclei_type_stat(args.pred_dir, args.true_dir,  args.root_dir, args.merge_epit)
        else:
            ...
            for i in range(0, args.cv):
                ...
                simple, full = run_nuclei_type_stat(pred_dir, true_dir,  args.root_dir, args.merge_epit)
